...
- **Overview watch list** (T194): pinned sessions (global, `stores/watch.ts`) render as interactive `TerminalInstance mirror` views. A mirror never resizes the PTY (renders at `terminal:get-size`, follows `terminal:resized`, shrinks the font) and strips terminal query replies from its input (`mirror-input.ts`) so the owning pane stays the only responder.
