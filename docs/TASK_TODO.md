...
23. TUI stretch in `fitAndSyncSize` never applies: `BASE_LETTER_SPACING` is 0 (so `0 * stretch`),
    and with `computeFit` ceiling the rows the line-height stretch is always 1. The dead strip on
    alt-screen TUIs (opencode, gemini, kiro, crush) is still there. Fix the math or remove it.
