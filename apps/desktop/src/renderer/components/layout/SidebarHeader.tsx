...
export function SidebarHeader() {
  return (
    <div className="flex h-10 items-center justify-between px-3 titlebar-drag">
      <span className="text-sm font-bold text-accent titlebar-no-drag">Exegol</span>
      <div className="ml-auto flex items-center gap-0.5 titlebar-no-drag">
        <button
          type="button"
          onClick={() => useAppStore.getState().toggleSidebar()}
          className="flex h-6 w-6 items-center justify-center rounded text-text-muted hover:bg-white/5 hover:text-text-secondary"
