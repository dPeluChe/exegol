import type { AgentStatus, ParallelRunStatus } from "@exegol/shared";
 ...
 *  - Sibling worktrees are NOT auto-deleted (user may inspect)
 *  - Broadcasts `parallel-run:changed` so any open comparator refetches
 */
export function promoteParallelAgent(db: Database.Database, runId: string, agentId: string): void {
  const run = getParallelRun(db, runId);
  if (!run) {
